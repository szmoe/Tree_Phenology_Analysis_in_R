...
There might be a problem with my temperature data, since I used the records from further location and patched it with the data from nearer locations. So, there is some inconsistencies between projections of different climate models. 
...
My plot got a problem because the model projected significantly lower chill days which leveled off after around 2050, but other climatic parameters are projected to be higher or similar patterns. As I mentioned before, this could be the data problem. I need to run with proper data to test this though.
...
![***A causal diagram of the relationship between temperature and bloom dates***](./pic/ch20_qa.png)

<br>

***3. What do we need to know to build a process-based model from this?***

The highlight of a process-based model is that it can incorporates extensive knowledge of all processes. Currently, phenology modelling frameworks primarily focus on the effects of temperature (chill/ heat accumulation). A process-based model would enable us to broaden the framework by incorporating details on tree physiology and other possible system drivers of phenology. 

Thus, a process-based model for the above causal diagram would require and could incorporate:

- long-term phenological data (e.g. bloom dates, leaf emergence dates)

- Long-term temperature records (daily/hourly)

- Other environmental data (data on precipitation, humidity, soil type, etc)

- Tree physiological data (e.g. species-specific chill and heat requirement, tree metabolic responses to temperature variations and photoperiod, etc.)

- Tree genetic data

These are some of the possible parameters, but a process-based model with many complex systems and parameters risks over-parameterization, which could make the model erroneous and unreliable. So, we should prioritize the parameters that are likely to be key drivers, or those for which we have a robust understanding, ensuring proper data availability (including for calibration and validation). 
...
# of x (the sign of a real number is 1, 0, or −1 if the number is positive, 
...
We can rule out the second reason because we do expect chill accumulations in apple trees in a location like Klein-Altendorf. This hypothesis may be true for trees in tropical or subtropical regions, where trees do not require much chill accumulation. 

So, it is possible that the PLS method does not work for this type of analysis. But then, we do see the PLS regression detecting chill response well in the case of 'Payne' walnuts in Davis, California. That means we can assume that the PLS method does work for this type of analysis, but just not all the time. California and Klein-Altendorf are two different locations with different climate&mdash; California has a Mediterran-like warmer climate, while Klein-Altendorf has much colder climate. This and other variations in the climatic and geographical characteristics between the two locations can be the reason behind the PLS not detecting the chill response in all occasions.
...
![***Results of a PLS analysis relating leaf emergence dates of ‘Payne’ walnuts in California to mean daily temperature***](./pic/ch22_6.png)
...
![***Results of a PLS analysis of bloom dates of cherries ‘Schneiders späte Knorpelkirsche’ in Klein-Altendorf, Germany, based on chill (in Chill Portions) and heat (in GDH) accumulation ([Luedeling et al., 2013a](https://www.sciencedirect.com/science/article/pii/S0168192313001780))***](./pic/ch24_7.png)
...
![***Results of a PLS analysis of apricot bloom in the southern UK, based on chill accumulation (in Chill Portions) and heat accumulation (in GDH) ([Martı́nez-Lüscher et al., 2017](https://www.sciencedirect.com/science/article/pii/S0168192317300503))***](./pic/ch24_8.jpg)
...
![***Results of a PLS analysis of leaf emergence dates of walnuts (cv. ‘Payne’) in Davis, California. Chill was quantified with the Dynamic Model, heat with the Growing Degree Hours Model ([Luedeling et al., 2013a](https://www.sciencedirect.com/science/article/pii/S0168192313001780))***](./pic/ch24_10.png)
...
location <- c("Beijing", "Beijing", "Beijing", "Beijing", "Shannxi",
              "Klein-Alterndorf", "California", "UK", "Croatia", "Tunisia", 
              "Tunisia")

cultivar <- c("Chestnut", "Jujube", "Apricot", "Mountain peach",
              "Apple", "Cherry", "Walnut","Apricot", "Grapevine", "Almond", 
              "Pistachios")

chilling_period <- c("Interrupted; unclear", "Interrupted; unclear", 
                     "Interrupted; unclear", "Interrupted; unclear",
                     "Interrupted; unclear", "Interrupted; unclear",
                     "Mostly continuous; clear", 
                     "Clear in later part of chill phase; long gap in between",
                     "Discontinuous clear phases", "Continuous; clear",
                     "Mostly continuous; clear")

forcing_period <- c("Continuous; clear", "Continuous; clear",
                    "Continuous; clear", "Continuous; clear",
                    "Clear with slight break", "Continuous; clear",
                    "Continuous; clear", "Continuous; clear",
                    "Continuous; clear", "Unclear end of forcing period",
                    "Interrupted; unclear")

table_cf <- data.frame(Location = location, Cultivar = cultivar,
                       Chilling_period = chilling_period,
                       Forcing_period = forcing_period)

colnames(table_cf) <- c("Location", "Cultivar", "Chilling period", "Forcing period")
...
The pattern I see here is that regions with colder climate (e.g. Beijing, Klein-Alterndorf) have unclear and discontinuous chilling period followed by clear continuous forcing period, while in regions with warmer climate (e.g. California, Tunisia), the chilling period can be clearly delineated, followed by less clear forcing period. 
...
**Reasons for the success for PLS analysis based on agroclimatic metrics are:**

- Availability of long phenological record

- Strong monotonic correlation between climate variables and phenological outcomes

- Proper choice of agroclimatic metrics for analysis (e.g. chilling hour vs. chill portion models)

- Temperate climate with clear delineation between chilling and forcing periods

<br>

**Reasons for the failure for PLS analysis based on agroclimatic metrics are:**

- Extreme climatic conditions (non-monotonic relationship)

- Low data variability (e.g. regions with all recorded temperatures fall within the effective chill range or out of the range)

- Unclear delineation of chilling and forcing periods (e.g. in cases where chill accumulation may not happen continuously but in interrupted phases due to complex interactions with other external variables&mdash; PLS is too simple for that)

- Sparse phenological data
...
![***Standard `chillR` output of phenology trend plot for pears ‘Alexander Lucas’ in Klein-Altendorf***](./data/OT/pheno_trend_plot.png)
...
Beijing has much colder winter but warmer summer than Klein Altendorf. So, Beijing has more seasonal temperature variations, and this may give a broader response plot. The contour lines for Beijing may be more wavy and less vertical than those for Klein Altendorf, as it has colder climate where forcing temperatures dominate phenology. Earlier bloom dates (blue zone) may also be observed near the top of the plot, showing more correlation with warm forcing period. 

Tunisia has a warmer climate than Klein Altendorf and a quick google search told me it has more temperature variations than Klein Altendorf. So, I would still expect a broader plot, but with more pronounced vertical lines, since in warmer climate, the chilling temperatures gain dominance on phenology as temperature increases. Earlier bloom dates (the blue zone) may be in lower part of the plot, and stronger color gradients along the horizontal direction&mdash; the blue (left side of the plot; lower x-axis temperature) and red zones (right side of the plot; higher x-axis temperature) may be like kinda parallel? 

Another important thing I don't consider here is the chilling requirements for Roter Boskoop apple cultivar. Based on the plots I saw during the lectures, the temperature response plots for different cultivars can look very different even in the same location. I understood it was due to their different chilling requirements. So, if I knew the range of chill requirements for Roter apple and more exact ranges of temperature variations for our locations of interest, then I might have a better speculation. But, should I assume that the temperature in Klein Altendorf is best suited for the Roter's chill requirements, since the data were collected from there? Is Roter Boskoop cultivar grown in Klein Altendorf because that's the best possible location for it? If that is the case, my speculation of response plots wouldn't be very far from the truth? Or would it?????? 
...
![***Bloom dates of cherries ‘Schneiders späte Knorpelkirsche’ in Klein-Altendorf, as a function of mean temperatures during the chilling and forcing phases ([Luedeling et al., 2013a](https://www.sciencedirect.com/science/article/pii/S0168192313001780?via%3Dihub))***](./pic/ch27_2.jpg)
...
![***Location of the study sites within the apple growing region of Shaanxi province in China. Mean monthly temperatures at the four locations are shown in the inset. “T” stands for mean annual temperature between 2001 and 2016.***](./pic/ch27_7.jpg)
...
Try it with adjusted ylim limit:

```{r 33.4.4c}

# Professor's note: In chapter 33, the reason why the full-bloom line is 
# interrupted is that you limited the y-axis extent to 75-140. 
# I suspect the latest full bloom date is later than day 140, so it gets cut off. 
# You didn't restrict the y-axis in your test plots, which is why this plotted 
# correctly there.
...
- We have seen in the previous chapter the importance of analyzing the risk of spring frost events in temperate fruit trees for historical periods. However, such analysis provides little information for planning strategies to adapt fruit orchards to potential frost events occurring in the future.

- What we actually need for supporting farmers in making decisions is an assessment of the probability of experiencing frost events (and ideally also their severity) in the future. Such an assessment should consider the prospective impacts of climate change, but it is not an easy task.

- On the one hand, spring frosts result from the occurrence of temperatures below the sensitivity threshold of the tree, which varies according to which kind of plant tissue is exposed to the cold. Plants become more sensitive to freezing temperatures as they develop from dormancy to harvest. 

- On the other hand, tree development during and after dormancy is itself driven by temperature, with advancing or delaying effects depending on particular factors such as the species, cultivar and ultimately the place.

- We therefore believe that conducting a proper frost risk analysis for future scenarios requires
    - a robust method for estimating future temperatures, and 
    - a valid modeling framework to predict bloom dates in temperate fruit trees.
    
\n

- We can combine dataset of past conditions with the simulated conditions. We then use this to determine to what extent the combined temperature domain of these datasets covers the climatic conditions that are projected for future climate scenarios.
...
unstructured_future_temps <- load_temperature_scenarios("data/Bonn/Bonn_future_climate",
                                                        "Bonn_future_")
...
# Create csv for 34.2c
write.csv(future_temps, "data/OT/future_temps.csv", row.names = FALSE)
...
enhanced <- read_tab("data/OT/final_weather_data_S1_S2_pear_hourly.csv")
...
write.csv(hull_temps_both,"data/Bonn/hull_temps_both.csv",row.names = FALSE)
write.csv(hull_temps_both_conference, "data/Bonn/hull_temps_both_conference.csv",row.names = FALSE)
...
hull_temps_both <- read_tab("data/Bonn/hull_temps_both.csv")
...
- This convex-hull plot is showing the coverage considering temperatures in CKA between 1958 and 2019 as well as all experimental seasons analyzed in chapter Experimentally enhanced PLS. 

- To make the phenology modelling approach work with historical and experimental data, we would need to use phenology records from the same species and cultivar (pear cultivar ‘Conference’). 

- Fortunately, we do have historical records for full bloom for this cultivar between 1958 and 2019. However, we may want to subset the experimental data used for the convex-hull plot to only retain seasons that were more or less normal. 

- This is to reduce the misleading effects we may have introduced by using extremely warm treatments.
...
historic_pheno_conference <- 
  read_tab("data/OT/Pheno_pear_conference_1958_2019.csv")[c("Year",
                                         "Full_bloom")]
...
# Load the data from the folder
exp_weather <- read_tab("data/OT/final_weather_data_S1_S2_pear_hourly.csv")

# Generate a new column (Year_2) to simulate the year and comply with the format of PhenoFlex functions
exp_weather["Year_2"] <- exp_weather$Treatment + exp_weather$Year + 3 # 1+2018+3

# Since this experiment was conducted during two consecutive seasons, the next step will fix a small continuity issue
# generated during the season 2
exp_weather[exp_weather$Treatment >= 17, "Year_2"] <-
  exp_weather[exp_weather$Treatment >= 17, "Year_2"] - 1 #remove 1 for no treatment?
...
# Import the phenology data from the repository
exp_pheno <- read_tab("data/OT/final_bio_data_S1_S2_pear.csv")
...
## For phenology and weather records, we now have the historical and experimental
## data in the same dataset. We can follow the same procedure we implemented in 
## the chapter Can we improve the performance of PhenoFlex? to fit the parameters 
## of PhenoFlex to data.
...
## Now we can apply what we learned in the chapter The PhenoFlex model and fit 
## the model parameters to data. Note that we start the fitting procedure with 
## wide ranges (particularly for yc and zc) in order to let the model find the 
## best estimates. We are saving the parameters and the predicted bloom.
...
write.csv(pheno_fit$par, "data/OT/PhenoFlex_hist_exp_pear.csv", row.names = FALSE)
...
write.csv(data.frame(pheno_merged[pheno_merged$Year %in% calibration_seasons, ],
                     "Predicted" = pheno_fit$pbloomJDays), "data/OT/PhenoFlex_hist_exp_predicted_bloom_pear.csv",
          row.names = FALSE)

```


- We can now feed our model with the fitted parameters and then estimate the prediction error by comparing predicted with observed bloom dates.

```{r 34.1d}

# Read the parameters
params <- read.csv("data/OT/PhenoFlex_hist_exp_pear.csv")[[1]]

# Generate a data set to collect the outputs of the fitting for the calibration data 
out_df <- read_tab("data/OT/PhenoFlex_hist_exp_predicted_bloom_pear.csv")

# Compute the error (observed - predicted)
out_df[["Error"]] <- out_df$pheno - out_df$Predicted

## We can now compute some model performance metrics based on the prediction 
## errors estimated above. This is not very relevant during the calibration 
## procedure, but it can give us an idea on how successful our calibration 
## attempt has been.

# Define the season used for calibration and validation in the PhenoFlex modelling approach
calibration_seasons <- out_df$Year
validation_seasons <- sort(pheno_merged[!(pheno_merged$Year %in% calibration_seasons), 
                                        "Year"])

# Define the list of seasons (weather data)
weather_season_list <- genSeasonList(weather_merged, mrange = c(9, 5), 
                                     years = calibration_seasons)

# Compute some metrics for the calibration procedure
calibration_metrics <-
  data.frame("Metric" = c("RMSEP",
                          "RPIQ"),
             "Calibration" = c(round(chillR::RMSEP(out_df$Predicted,
                                           out_df$pheno,
                                           na.rm = TRUE), # remove all NA values
                                     2), # round to 2 digits
                               round(chillR::RPIQ(out_df$Predicted,
                                          out_df$pheno),
                                     2)))
...
### <span style="color:#71797E">34.2</span>&nbsp; Validation

<div style="height: 18px;"></div>
    
- We should probably now take a look at how well this phenology model can predict bloom dates for seasons not used in the calibration data set. 
...
validation_metrics <- data.frame(calibration_metrics,
                                 "Validation" = c(round(chillR::RMSEP(valid_df$Predicted,
                                                              valid_df$pheno,
                                                              na.rm = TRUE),
                                                        2),
                                                  round(chillR::RPIQ(valid_df$Predicted,
                                                             valid_df$pheno,
                                                             na.rm = TRUE),
                                                        2)))

write.csv(validation_metrics, "data/OT/frost_validation_metrics.csv", row.names = FALSE)

validation_metrics <- read_tab("data/OT/frost_validation_metrics.csv")
...
- The results shown in the table above represent a good preliminary performance of the model in forecasting bloom dates in pear trees for conditions we may expect in the future. 

- Note that for a more robust analysis, we should try fitting `PhenoFlex` many times in order to achieve lower `RMSEP` values. We could also consider limiting the number of experimental seasons (which may be adding somewhat ‘extreme’ values) to narrow the range of conditions observed in the warm part of the model validity domain. 

- Additional errors may originate from merging two data sets obtained with different methodologies (mature trees in the orchard vs. young potted trees, different observers across experiments, etc.). 

- Let’s take a look at the fitting outcomes. We added labels to show the season presenting an error greater than 10 days.
...
- Using the set of parameters fitted in the previous model runs, we can predict bloom dates for future scenarios. To this end, we will need to use the data generated in chapter **Future temperature scenarios**. Fortunately, we already loaded that data into R when generating the convex-hull plots.
...
# Load future_temps data
future_temps <- read_tab("data/OT/future_temps.csv")
...
write.csv(future_bloom, "data/OT/frost_future_bloom.csv", row.names = FALSE)

## We can now implement the same procedure to estimate bloom dates for 
## historical simulated scenarios.

# Bloom for past simulated scenarios ####
# Load the historical simulated scenarios
temps_past_scenarios <- load_temperature_scenarios("data/Bonn",
                                    "Bonn_hist_scenarios")
...
write.csv(simulated_bloom, "data/OT/frost_simulated_bloom.csv", row.names = FALSE)

```

- We are going to produce a plot now to show the likely bloom dates for historical and future scenarios. We will also add the data used for calibration of the PhenoFlex model.

- The plot is a bit complicated, because it’s difficult to plot two datasets with different axes in the same plot panel. 

- The plotting strategy below goes as follows:
...
simulated_bloom <- read_tab("data/OT/frost_simulated_bloom.csv")
future_bloom <- read_tab("data/OT/frost_future_bloom.csv")
...
ggsave("pic/future_pheno_pear.png", 
       device = "png", 
       width = 19, 
       height = 15,
       units = "cm", 
...
- We can see that bloom in pear cv. Conference is expected to advance slightly under future scenarios. 

- What we need to know now is the probability of observing frost events during the flowering period. 

- To this end, we will assume a bloom period of 10 days around the expected date for full bloom for each simulated year and take a look at the temperatures during these days.

```{r 34.2e, eval=FALSE}

## Frost risk part
# Add the "buffer" around the estimated bloom dates to compute the number of 
# hours below 0 °C in this period. Hypothetically, this represents the
...
## We will now compute the number of frost hours expected to occur under future 
## scenarios as well as under historical simulated scenarios. 
## We implement this by using several 'for' loops.
...
# Save as csv, so I don't need to run the loop again
write.csv(simulated_frost, "data/OT/simulated_frost.csv", row.names = FALSE)
...
# Save as csv, so I don't need to run the loop again
write.csv(future_frost, "data/OT/future_frost.csv", row.names = FALSE)

```


- We can now plot the results to see the overall picture. 

- We will use the same kind of plot we normally use to show the projections of
agro-climatic-related metrics. In brief, we create two plots and combine them using the `patchwork` package.

```{r 34.2f}

# Load saved csv for plotting
simulated_frost <- read_tab("data/OT/simulated_frost.csv")
future_frost <- read_tab("data/OT/future_frost.csv")
...
- One interesting feature here is the large number of outliers we generated for each simulated scenario. 

- The overall trend suggests there may be a slight decline in the risk of spring frost events for pear cv. Conference under future scenarios although the projections for bloom suggested an advance in the occurrence of the flowering period.

<br>

## Chapter 35: Major concepts

### <span style="color:#71797E">35.1</span>&nbsp; Key insights

#### <span style="color:#71797E">35.1.1</span>&nbsp; Tree dormancy
...
#### <span style="color:#71797E">35.1.2</span>&nbsp; Climate change
...
- The intensity of future warming will depend on atmospheric greenhouse gas concentrations. Since these can’t be known in advance, we use distinct scenarios&mdash;Shared Socioeconomic Pathways (SSPs) at present&mdash;to cover the range of possibilities.

- To mitigate climate change, we must cut our greenhouse gas emissions, particularly in the energy sector!

<br>

#### <span style="color:#71797E">35.1.3</span>&nbsp; Phenology modeling
...
#### <span style="color:#71797E">35.1.4</span>&nbsp; Phenology responses to global warming
...
#### <span style="color:#71797E">35.1.5</span>&nbsp; The `PhenoFlex` modeling framework
...
### <span style="color:#71797E">35.2</span>&nbsp; Major concepts we’ve encountered
